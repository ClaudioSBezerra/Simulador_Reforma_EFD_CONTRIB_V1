...
const EnergiaPanel: React.FC<EnergiaPanelProps> = ({ spedData, taxRates, year, branches }) => {
  ...
  const sourceList = tab === 'credits' ? spedData.c500 : spedData.c600;

  const filtered = sourceList.filter(rec => {
    return cnpjFilter === 'all' || rec.cnpj === cnpjFilter;
  });
      ...
      <div className="flex flex-wrap gap-4 items-center bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
        <div className="flex bg-gray-50 p-1.5 rounded-2xl border border-gray-100">
          <button 
            ...
            className={`px-8 py-2.5 rounded-xl text-sm font-black transition-all uppercase tracking-widest ${tab === 'credits' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-400 hover:text-gray-600'}`}
          >
            Créditos (C500)
          </button>
          <button 
            onClick={() => setTab('debits')}
            className={`px-8 py-2.5 rounded-xl text-sm font-black transition-all uppercase tracking-widest ${tab === 'debits' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-400 hover:text-gray-600'}`}
          >
            Débitos (C600)
          </button>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-8 py-12 text-center text-gray-400 italic">Nenhum registro para {tab === 'credits' ? 'C500' : 'C600'} com os filtros atuais.</td>
                </tr>
              )}
            </tbody>
          </table>
